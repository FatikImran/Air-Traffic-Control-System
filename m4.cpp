...
void showSimulationStatus(const vector<Aircraft*>& aircrafts, const vector<Runway>& runways, int elapsedTime);
        ...
        switch (choice) {
            case 1: // Start Simulation
                if (!simulationRunning) {
                    ...
                    startSimulation(airlines, runways, activeAircrafts, aircraftSequence);
                } else {
            ...
            case 2: // End Simulation
                if (simulationRunning) {
                    ...
                    cout << "No simulation is running!" << endl;
                }
                break;
            case 3: // Input Flight Data
                if (!airlines.empty()) {
                    inputFlightData(airlines, activeAircrafts, aircraftSequence);
                } else {
                    cout << "Please start the simulation first to initialize airlines!" << endl;
                }
                break;
            case 4: // Show Simulation Status
                if (simulationRunning) {
                    showSimulationStatus(activeAircrafts, runways, 0);
                } else {
                    cout << "No simulation is running!" << endl;
                }
                break;
            case 5: // Exit
                cout << "Exiting AirControlX. Goodbye!" << endl;
                // Clean up
                for (auto aircraft : activeAircrafts)
                    removeAircraftFromEverywhere(aircraft, activeAircrafts, runways);
                return 0;
...
void displayMenu() {
    ...
    cout << "3. Input Flight Data\n";
    cout << "4. Show Simulation Status\n";
    cout << "5. Exit\n";
}
...
void inputFlightData(vector<Airline>& airlines, vector<Aircraft*>& activeAircrafts, int& aircraftSequence) {
    ...
    cout << "Flight " << flightNumber << " added successfully.\n";
}
...
void startSimulation(vector<Airline>& airlines, vector<Runway>& runways, vector<Aircraft*>& activeAircrafts, int& aircraftSequence) {
    ...
    while (elapsedTime < SIMULATION_DURATION && simulationRunning) {
...
void showSimulationStatus(const vector<Aircraft*>& aircrafts, const vector<Runway>& runways, int elapsedTime) {
    displayStatus(aircrafts, runways, elapsedTime);
}
...
void displayStatus(const vector<Aircraft*>& aircrafts, const vector<Runway>& runways, int elapsedTime) {
    ...
    for (const auto& aircraft : aircrafts) {
        ...
        cout << aircraft->flightNumber << " (" << aircraft->airline->name << "): "
             ...
             << ", BasicPhase: " << getPhaseName(aircraft->phase)
